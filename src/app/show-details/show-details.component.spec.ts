...
describe('ShowDetailsComponent', () => {
  ...
  it('should call API to get Cast details', () => {
    spyOn(ApiService.prototype, 'getShowCast').and.returnValue(of(JSON.parse(castMockData)));
    component.ngOnInit();
    expect(typeof (component.castDetails)).toBe('object');
  });

  it('should show error when API call returns an error for getShowCast', () => {
    spyOn(ApiService.prototype, 'getShowCast').and.returnValue(throwError('error'));
    component.ngOnInit();
    expect(component.hasError).toBeTruthy();
  }); 
});
